...
- 同一俱乐部、达到 65% 晋级线、随后进入相邻高一级组；
- 同一具有区分度的俱乐部、前场未达到 65% 晋级线、随后仍在同一规范组别参赛。

第二类不是“同名＋同俱乐部”即合并：前后必须是不同赛事，日期须能确定先后
（精确日期，或不同年份），间隔不超过 24 个月，并且不存在性别、出生年、同场
占两个席位等硬冲突。包含三条以上记录的展示组还必须有一个全组共同的特色
俱乐部，禁止通过中间记录把两个不同俱乐部的同名者传递合并。省市名、“中国”、
`A2` 等普通标签不能作为特色俱乐部。
永久链接仍需把被并入实体的每个 sighting 指向选定 canonical domestic ID：

```csv
sighting,sighting-被并入记录,domestic,domestic-选定主实体,reviewed-high,club+promotion-continuity,,maintainer,2026-07-19,
```

如果证据只达到高分、但同一年内只有年份而无法证明先后，则优先审核但不默认
聚合。明确不是同一人时不要“降分后放回队列”，而要写负链接，防止以后重新成组。
