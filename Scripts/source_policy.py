...
conservative by default: responses and parsed event data stay outside the Git
worktree unless a future, documented authorization explicitly changes the
policy.  Merely running on a residential IP is not publication permission.
"""
...
def chess_results_release_policy() -> str:
    value = os.environ.get(CHESS_RESULTS_POLICY_ENV, "link-only").strip().lower()
    return value if value in {"link-only", AUTHORIZED_VALUE} else "link-only"


def require_chess_results_publication() -> None:
    if chess_results_release_policy() == AUTHORIZED_VALUE and os.environ.get(
        "CHESS_RESULTS_PUBLICATION_AUTHORIZED"
    ) == "1":
        return
    raise SourcePolicyError(
        "COMPLIANCE_POLICY_BLOCKED",
        "Chess-Results 当前为 link-only：原始页面、赛事数据库和 PGN 只能保存在本地私有运行区。",
    )
